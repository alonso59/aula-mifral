<script>
	...
	const updateHandler = async (_group) => {
		const res = await updateGroupById(localStorage.getItem('token'), group.id, _group).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	const deleteHandler = async () => {
		const res = await deleteGroupById(localStorage.getItem('token'), group.id).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
