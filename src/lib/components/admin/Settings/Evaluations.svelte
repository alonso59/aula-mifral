<script lang="ts">
	...
	const submitHandler = async () => {
		evaluationConfig = await updateConfig(localStorage.getItem('token'), evaluationConfig).catch((err) => {
			toast.error(err);
			return null;
		});
	...
	onMount(async () => {
		if ($user?.role === 'admin') {
			evaluationConfig = await getConfig(localStorage.getItem('token')).catch((err) => {
				toast.error(err);
				return null;
			});
		}
