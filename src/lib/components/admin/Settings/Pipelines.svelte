<script lang="ts">
	...
	const setPipelines = async () => {
		...
		if (PIPELINES_LIST.length > 0) {
			...
			pipelines = await getPipelines(localStorage.getItem('token'), selectedPipelinesUrlIdx);
	...
	const uploadPipelineHandler = async () => {
		...
		if (pipelineFiles && pipelineFiles.length !== 0) {
			...
			const res = await uploadPipeline(localStorage.getItem('token'), file, selectedPipelinesUrlIdx).catch(
				(error) => {
					console.error(error);
	...
	onMount(async () => {
		PIPELINES_LIST = await getPipelinesList(localStorage.getItem('token'));
