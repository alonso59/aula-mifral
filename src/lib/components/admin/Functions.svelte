<script lang="ts">
	...
	const shareHandler = async (func) => {
		const item = await getFunctionById(localStorage.getItem('token'), func.id).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	const cloneHandler = async (func) => {
		const _function = await getFunctionById(localStorage.getItem('token'), func.id).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	const deleteHandler = async (func) => {
		const res = await deleteFunctionById(localStorage.getItem('token'), func.id).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	const toggleGlobalHandler = async (func) => {
		const res = await toggleGlobalById(localStorage.getItem('token'), func.id).catch((error) => {
			toast.error(`${error}`);
		});
...
<ImportModal
	...
	loadUrlHandler={async (url) => {
		return await loadFunctionByUrl(localStorage.getItem('token'), url);
	}}
	onImport={(func) => {
		sessionStorage.function = JSON.stringify({
			...func
				...
				<div class=" self-center mx-1">
					<Tooltip content={func.is_active ? $i18n.t('Enabled') : $i18n.t('Disabled')}>
						<Switch
							...
							on:change={async (e) => {
								toggleFunctionById(localStorage.getItem('token'), func.id);
								models.set(
									await getModels(
										localStorage.getItem('token'),
	...
	<div class="flex space-x-2">
		...
		{#if $functions.length}
			<button
				class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 transition"
				on:click={async () => {
					const _functions = await exportFunctions(localStorage.getItem('token')).catch((error) => {
						toast.error(`${error}`);
						return null;
					});
	...
	on:confirm={() => {
		...
		reader.onload = async (event) => {
			...
			for (const func of _functions) {
				const res = await createNewFunction(localStorage.getItem('token'), func).catch((error) => {
					toast.error(`${error}`);
					return null;
				});
			}
